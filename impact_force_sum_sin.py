...
for i in range(num_impacts):
    ...
    # 生成当前冲击的波形
    impact_wave1 = np.sin(omega_sine * (impact_time_values - i * delta_t))
    impact_wave2 = np.zeros(num_points)
    each_point = int(num_points / 2) 
    print('each_point=', each_point)
    base_array_front = np.linspace(0, 1, each_point)
    base_array_end = np.linspace(1, 0, each_point)
    impact_wave2 = np.concatenate([base_array_front, base_array_end[1:]])
    
    # 对当前波形进行零填充，使其对齐到总时间轴
    padded_impact_wave = np.pad(impact_wave2, (i * num_samples_per_impact, num_total_points - num_points - i * num_samples_per_impact), 'constant', constant_values=0)
