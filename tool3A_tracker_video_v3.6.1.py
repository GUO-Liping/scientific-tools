...
# 预设高对比度色彩库，用于区分不同的追踪点
TRACK_COLORS = [
    "#ff4d4f", "#5ac8fa", "#ffeb3b", "#8cc63f", "#c678dd",
    "#f7931e", "#00ffff", "#ff5c8a", "#00ff00", "#ff9800",
    "#9e005d", "#4caf50", "#3f51b5", "#e91e63", "#00bcd4"
]
...
class VideoWidget(QWidget):
    ...
    def _paint_overlays(self, painter, view_rect):
        ...
        if state.get("mode") == "SET_COORD_2" and coord_origin and state.get("mouse_curr_pos"):
            p1 = self._to_screen_point(coord_origin)
            p2 = self._to_screen_point(state["mouse_curr_pos"])
        ...
        if coord_origin:
            ...
            # 屏幕坐标下的方向向量 (屏幕Y向下，所以角度的sin取负)
            ux = math.cos(coord_angle)
            uy = -math.sin(coord_angle)
            vx = -math.sin(coord_angle)
            vy = -math.cos(coord_angle)

            # X轴
            pX1 = self._to_screen_point((coord_origin[0] - L*ux, coord_origin[1] - L*uy))
            pX2 = self._to_screen_point((coord_origin[0] + L*ux, coord_origin[1] + L*uy))
            painter.setPen(QPen(QColor(255, 159, 28, 180), 1.5))
            painter.drawLine(pX1, pX2)
            
            # Y轴
            pY1 = self._to_screen_point((coord_origin[0] - L*vx, coord_origin[1] - L*vy))
            pY2 = self._to_screen_point((coord_origin[0] + L*vx, coord_origin[1] + L*vy))
            painter.setPen(QPen(QColor(76, 175, 80, 180), 1.5)) 
            painter.drawLine(pY1, pY2)

            # 绘制坐标系标签
            painter.setPen(QPen(QColor("#ff9f1c")))
            label_X = self._to_screen_point((coord_origin[0] + 120*ux, coord_origin[1] + 120*uy))
            painter.drawText(label_X + QPointF(5, -5), "X")
            painter.setPen(QPen(QColor("#4caf50")))
            label_Y = self._to_screen_point((coord_origin[0] + 120*vx, coord_origin[1] + 120*vy))
            painter.drawText(label_Y + QPointF(5, -5), "Y")

            draw_point(coord_origin, "#ff4d4f", 5)
            painter.setPen(QPen(QColor("#ff4d4f")))
            painter.drawText(origin_scr + QPointF(10, 15), f"Origin (0,0)\n{math.degrees(coord_angle):.1f}°")
        ...
        for i, trail in enumerate(all_trails):
            ...
            for j in range(1, len(trail)):
                p1 = self._to_screen_point(trail[j-1])
                p2 = self._to_screen_point(trail[j])
                painter.drawLine(p1, p2)
            for pt in trail:
                draw_point(pt, color_hex, 2)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("高速视频运动分析工具 V4.5 (多点追踪+自定义坐标系)")
        ...
        self.coord_angle = 0.0 # CCW from horizontal in radians
    ...
    def create_actions(self):
        ...
        self.video_actions = [
            self.act_calib, self.act_coord, self.act_mark, self.act_grid_size,
    ...
    def _init_plot_curves(self, num_points):
        ...
        for i in range(num_points):
            c_hex = TRACK_COLORS[i % len(TRACK_COLORS)]
            pen = pg.mkPen(c_hex, width=2)
            brush = pg.mkBrush(c_hex)
            
            c_x = self.plot_x.plot(pen=pen, symbol='o', symbolSize=5, symbolBrush=brush)
            c_y = self.plot_y.plot(pen=pen, symbol='o', symbolSize=5, symbolBrush=brush)
    ...
    def recalc_all_marks(self):
        """核心重算逻辑：修改坐标系后，遍历并更新所有已捕捉点的物理相对坐标"""
    ...
    def on_video_clicked(self, x, y):
        ...
        if self.state == "DRAW_AUX":
            ...
            if ok1 and real_dist > 0:
                ...
                if ok3 and grid_sz > 0: self.grid_size = grid_sz
        ...
        # ---------- 新增：设置坐标原点与旋转角度 ----------
            ...
            dy = self.coord_origin[1] - click_y # 注意物理Y轴与屏幕Y轴方向相反
            ...
            if ok:
                ...
                self.recalc_all_marks() # 核心：修改坐标系后全局重算！
